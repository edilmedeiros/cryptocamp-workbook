  ...
  \begin{center}
    \begin{tcolorbox}[width=10cm]
      \begin{pchstack}
        ...
        \pchspace
        \procedure[headlinesep=1pt]{Oracle $\pcoracle{H}(x)$}{%
          \gamechange{$\pcassert \text{$x$ does not end with $r$}$} \\
          \pcif T[x] = \bot \pcthen \\
          \t T[x] \sample \{0,1\}^\secpar \\
          \pcreturn T[x]
        }
...
Since the distribution of the random oracle output is identical to the distribution of $c'$ when the adversary doesn't query a value ending with $r$, the games produce the same output except when the assertion is triggered.
