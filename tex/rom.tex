...
\begin{lemma}\label{lem:ro-indistinguishable}
  ...
  \[
  \left|\Pr[\game{\algo{ROExample}}{} = 1] - \Pr[\game{\algo{ROExample}_2}{} = 1]\right| \leq \frac{q}{2^\secpar}
  \]
  where $\game{\algo{ROExample}_2}{}$ is defined in \autoref{fig:ro-game-hop}.
\end{lemma}

\begin{remark}
  Unlike the security games we have seen so far where there are winning and losing conditions, $\algo{ROExample}$ simply runs the adversary and returns whatever the adversary outputs.
  \autoref{lem:ro-indistinguishable} simply states that the adversary's behavior (its output distribution) is identical in both games with probability at least $1 - q/2^\secpar$.
\end{remark}
...
\begin{figure}[tbhp]
  \begin{center}
    \begin{tcolorbox}[width=8cm]
      \begin{pchstack}
        \procedure[headlinesep=1pt]{$\game{\Game~\algo{ROExample}_2}{}$}{%
          T \defeq \emptyset \\
          r \sample \{0,1\}^\secpar \\
...
\begin{proof}
  By inspection of the code, we observe that $\game{\algo{ROExample}_2}{}$ is identical to $\game{\algo{ROExample}}{}$ except that:
  \begin{enumerate}
...
\begin{mysolution}
  \begin{enumerate}
    ...
    \[
    \advantage{Hide}{\adv, \algo{HashCom}_y} \leq \Pr[\algo{H.Eval}(0) = y] + \advantage{Hide}{\adv, \algo{HashCom}} \leq \frac{1}{2^\secpar} + \frac{q}{2^\secpar}
    \]
...
\begin{figure}[h]
  \begin{center}
    \begin{tcolorbox}[width=10cm]
      \begin{pchstack}
        \procedure[headlinesep=1pt]{$\game{\Game~\algo{ComHide}_2}{\algo{HashCom}}$}{%
          \params \gets \algo{Setup}(1^\secpar) \\
...
Note that in $\game{\algo{ComHide}_2}{\algo{HashCom}}$, the value $c'$ is uniformly random and independent of $b$, so the adversary's view is independent of $b$.
Therefore:
\[
\Pr[\game{\algo{ComHide}_2}{\algo{HashCom}} = 1] = \frac{1}{2}
\]

The games differ only when $\adv$ queries some value to the oracle that ends with $r$.
This is because in $\game{\algo{ComHide}}{\algo{HashCom}}$, the challenger queries $\pcoracle{H}(m_b \| r)$ to compute the commitment, while in $\game{\algo{ComHide}_2}{\algo{HashCom}}$, it samples $c'$ uniformly at random without querying the oracle.
...
However, if the adversary does query $m_b \| r$ (or any other value ending with $r$), the games diverge: in $\game{\algo{ComHide}}{\algo{HashCom}}$, the oracle returns the value consistent with $c$, while in $\game{\algo{ComHide}_2}{\algo{HashCom}}$, the oracle returns a fresh random value.
...
\begin{align*}
  ...
  &= \left|\Pr[\game{\algo{ComHide}}{\algo{HashCom}} = 1] - \Pr[\game{\algo{ComHide}_2}{\algo{HashCom}} = 1]\right| \\
